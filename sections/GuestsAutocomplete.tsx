 ...
 */
const isGuestPropKey = (key: string): key is GuestsKeys => key in guests.$inferInsert;
/**
 * Checa se `value` é do mesmo tipo de guests[key]
 */
const isGuestPropType = (key: GuestsKeys, value: unknown): value is GuestValue<typeof key> => typeof value === typeof guests.$inferInsert[key];
interface Props {
    mode?: "search" | "confirm";
    email?: string;
}
...
export async function loader({ mode = "search" }: Props, req: Request, { invoke }: AppContext) {
...
export async function action(props: Props, req: Request, ctx: AppContext & RecordsApp): Promise<Props> {
    ...
    return (<div>
      <div id="page">
        <div class="flex gap-2">
          ...
          <input name={Querystring} id={Querystring} value={props.searchValue ?? ""} hx-post={searchUrl} hx-trigger={`input changed delay:500ms`} hx-target="closest section" hx-swap="outerHTML" class="border border-gray-300 rounded"/>
        </div>
      </div>

      <div class="divide-y divide-gray-300 p-2 w-fit">
        {props.guests.map((guest) => {
            return (<form hx-post={useComponent(import.meta.url, props)} hx-trigger="click" hx-target="closest section" hx-swap="outerHTML" class="flex flex-col gap-2">
              <input type="text" value={guest.name ?? ""} disabled/>
              <input type="email" value={guest.email ?? ""} disabled/>

              <label>
                <input type="checkbox" name="confirmed" checked={guest?.confirmed ?? false}/>
                Confirmado?
              </label>

              <label>
                <input type="checkbox" name="plusOne" checked={guest?.plusOne ?? false}/>
                Tem convidado?
              </label>

              {guest?.plusOne && (<input name="plusOneName" value={guest?.plusOneName ?? ""} placeholder="Plus One Name"/>)}
      ...
      <script type="module" dangerouslySetInnerHTML={{
            __html: useScript(script, Querystring, Querystring),
        }}/>
    </div>);
}
