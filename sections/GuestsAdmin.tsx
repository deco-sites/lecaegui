...
export async function loader({ mode, email }: Props, req: Request, { invoke }: AppContext) {
    ...
    if (mode === "create" && req.body) {
        ...
        formData.forEach((value, key) => isGuestPropKey(key) &&
...
export default function ManageGuests({ guests = [] }: SectionProps<typeof loader>) {
    ...
    return (<>
      <div>
        <form hx-post={createUrl} hx-trigger="click" hx-target="closest section" hx-swap="outerHTML" class="p-2 flex flex-col gap-2">
          <div class="flex gap-2">
            <label for="name">Name</label>
            <input 
    // propriedade name do guests
    name="name" id="name" required class="border border-gray-300 rounded"/>
          </div>
    ...
    name="email" id="email" required class="border border-gray-300 rounded"/>
          </div>
      ...
      <div class="divide-y divide-gray-300 p-2 w-fit">
        ...
        {guests.map((guest) => {
            ...
            return (<div class="flex gap-2 items-center">
              ...
              <form hx-post={guestDeleteUrl} hx-trigger="click" hx-target="closest section" hx-swap="outerHTML" class="w-4 h-4">
